...
[reference/dataModel.yaml](skill/d360-segments-activations/reference/dataModel.yaml) ·
[reference/before-using-and-on-data-model-changes.md](skill/d360-segments-activations/reference/before-using-and-on-data-model-changes.md) ·
...
The governance surface — `SKILL.md`, the semantic layer (`dataModel.yaml`), the segment/validation
...
Treat `dataModel.yaml` and the skill like code that gets **reviewed and released**, not a file that
...
1. **Author + review in git.** A contributor opens a PR (a marketer may *propose* a `VERIFY` entry;
   ...
   same governance bar the runbook already sets for `SKILL.md` and `dataModel.yaml`. This is exactly
...
| `reference/dataModel.yaml` (semantic layer) | Per-user OAuth tokens (PKCE login) |
...
> *physical schema* — object and field API names, nothing more. `dataModel.yaml` is a human-authored
...
- **Version the contract.** Tag releases (e.g. `datamodel-v3`, or bump `version:` in
  `dataModel.yaml`). An environment can pin a known-good version and upgrade on purpose rather than
...
| YAML merge conflicts on `dataModel.yaml` | One canonical file, small focused PRs, architect as owner; treat like any reviewed code change. |
...
1. Make the **git repo the single source of truth** for the whole skill (not just `dataModel.yaml`),
...
3. Keep the **author → PR → architect-verify → merge** loop as the *only* way the contract changes
   ...
   [before-using-and-on-data-model-changes.md](skill/d360-segments-activations/reference/before-using-and-on-data-model-changes.md)).
4. When the user base grows, evaluate **rung 3** — serving `dataModel.yaml` from a single location we
