...
| Semantic layer verified against the sandbox (`VERIFY → verified`) | [dataModel.yaml](skill/d360-segments-activations/reference/dataModel.yaml) | ☐ not started |
| OCL/Snowflake benchmark locked | [validation/](skill/d360-segments-activations/validation/) | ☐ not started |
...
| 2026-07-28 | Make the skill **self-contained**: move `reference/`, `validation/`, `feedback/` **inside** `skill/d360-segments-activations/` and rewrite all links to be relative to the skill folder | Enterprise-managed Claude blocks user-added connectors, so the governed skill must be **published org-wide by an admin from git** as one portable folder. A self-contained folder resolves its links identically in the repo checkout and when deployed standalone, keeps git the single source of truth, and stays tool-agnostic (drops unchanged into the Cursor/CLI fallback if MCP is disallowed) |
| 2026-07-27 | Add a **self-improvement session log** ([feedback/](skill/d360-segments-activations/feedback/)): the agent captures friction (repeated clarifications, failures, gaps, workarounds) to a notebook; the **skill owner** reviews and makes the one canonical skill edit. Toggle in `SKILL.md`: **`self-tune`** during Phase 0 build (agent may propose edits via PR), **`log-only`** default in production (log only). | Lets the skill self-improve during the POC and keep improving in users' hands, **without** each machine drifting to a different skill version — improvements ship through the same one-canonical-copy repo loop as `dataModel.yaml` ([scaling-via-repo.md](scaling-via-repo.md)) |
| | | |
