...
5. **Populate & verify the semantic layer** — fill `dataModel.yaml` from the **live org, as the runtime principal (a user's permission set)**; flip every `VERIFY` → `verified`. Align `pii:` flags to the org's data classification.
